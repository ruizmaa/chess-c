...
int is_valid_move(const ChessBoard* board, const int from_row, const int from_col, const int to_row, const int to_col) {
    ...
    if (is_friendly_occupied(piece->color, board->squares[to_row][to_col])) return 0;  // Hay una pieza amiga en el destino
    ...
    switch (piece->type) {
        ...
        case KING:
            ...
            // Enroque
            // if (!valid) {
            //     valid = is_valid_castling_move(board, from_row, from_col, to_row, to_col);
            // }
            break;
        default:
            return 0;
...
int is_valid_pawn_move(const ChessBoard* board, const int from_row, const int from_col, const int to_row, const int to_col) {
    ...
    if (to_row == from_row + direction && (to_col == from_col + 1 || to_col == from_col - 1)) {
        if (board->squares[to_row][to_col] != NULL && board->squares[to_row][to_col]->color != pawn_color) {
            return 1;
        }
    }
...
int is_valid_bishop_move(const ChessBoard* board, const int from_row, const int from_col, const int to_row, const int to_col) {
    ...
    // Verificar piezas intermedias
    for (int i = 1; i < distance; i++) {
        if (board->squares[from_row + i * row_step][from_col + i * col_step] != NULL) return 0; // Hay una pieza bloqueando el camino
    }
