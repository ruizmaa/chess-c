...
void play_turn(ChessBoard* board) {
    ...
    while (1) {
        ...
        if (!is_valid_from_piece(selected_piece, turn, from)) continue;         // Validar que la coordenada sea válida (haya una pieza del jugador)
...
void change_turn (ChessBoard* board){
    if (board->status.current_turn == BLACK) board->status.current_turn = WHITE;
    else  board->status.current_turn = BLACK;
